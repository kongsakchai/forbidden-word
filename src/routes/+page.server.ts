import { userStore } from '$lib/stores/user-store.js';
import { env } from '$env/dynamic/private';

const images = [
	'https://us-tuna-sounds-images.voicemod.net/8fab1c3e-1cb9-47b6-8fe3-699bc7c2aaa9-1655706403347.jpg',
	'https://play-lh.googleusercontent.com/cJokjWYV_EhTZJvJG0zbV6CowN5V8EoyjzF4LssGyjhTo6rAVntx3XjD7AkBQ2IdFKw',
	'https://uploads.dailydot.com/2018/10/olli-the-polite-cat.jpg?q=65&auto=format&w=2270&ar=2:1&fit=crop',
	'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSjpv6UYRh7Rc2cn27aE7oGjdMKa6IfJy6KQtIsBDtHPGpjpLTQzOrBK1zUdWSdWyWdFMk&usqp=CAU',
	'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQrskwmGEPlbLpV4Sd52GYVaTi5J9giNka1IA&usqp=CAU',
	'data:image/jpeg;base64,',
	'https://cloudfront-us-east-1.images.arcpublishing.com/metroworldnews/T5NH6ZQY6BED3PMIC5F63HWMC4.jpg'
];

export const load = async () => {
	const random = Math.round(Math.random() * (images.length - 1));
	const url_image = images[random];

	const data: string[] = await fetch(env.URL_WORD)
		.then((res) => res.json())
		.then((data) => data);

	const randomW = Math.round(Math.random() * (data.length - 1));

	return { url_image, word: data[randomW] };
};

export const actions = {
	save: async ({ request, cookies }) => {
		const data = await request.formData();
		const name = data.get('name') as string;

		const id = Math.round(Math.random() * name.length * 9999).toString();
		const user = { name, id };

		cookies.set('user', JSON.stringify(user));
		userStore.set(user);

		return { user };
	}
};
